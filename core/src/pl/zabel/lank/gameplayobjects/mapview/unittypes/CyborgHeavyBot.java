...
public class CyborgHeavyBot extends RepairingUnit{
	
	protected TextureRegion buildLaser;
	protected float buildLaserAngle, laserLenght;
	
	public CyborgHeavyBot(byte faction, MapView mapView, UnitBuilder builder) {
		super("Ciezki Bot", faction, mapView, builder);
		this.setSize(150, 90);
		findRegionForSpecificUnit();
		this.imageName="cyborg-heavy-bot-image";
		...
		this.setSpeed(65);
		this.maxHitPoints = 2600;
		this.currentHitPoints = 2600;
		...
		if(this.getFaction()==(byte)0)
		{
			...
			setUpGunParameters(30, 30, 100, 0.5f*MapSession.getUnitShootSpeedMod(), 
					600*MapSession.getBotBulletSpeedMod(), 800, "heavyBot");
			this.repairSpeed = MapSession.getUnitRepairSpeedMod();
		}
		else
			setUpGunParameters(30, 30, 100, 0.5f, 600, 800, "heavyBot");
	...
	{
		this.displayTexture = MapUnitTextureContainer.findRegionCyborgHeavyBot(atlasKey);
		this.displayTextureTurret = MapUnitTextureContainer.findRegionCyborgHeavyBot(turretAtlasKey);
	}
	@Override
	public void schemeActivationEvent() {
		SoundEffectManager.playSmallClickSound();
		this.builder.addToQueue(new CyborgHeavyBot(this.getFaction(), mapView, builder));		
	}
