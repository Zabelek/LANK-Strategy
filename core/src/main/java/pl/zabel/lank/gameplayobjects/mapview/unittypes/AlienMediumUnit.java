...
public class AlienMediumUnit extends MobileUnit{
	
	public AlienMediumUnit(byte faction, MapView mapView, UnitBuilder builder) {
		super("Jednostka Obcych", faction, mapView, builder);
		this.setSize(120, 70);
		findRegionForSpecificUnit();
		setUpGunParameters(30, 30, 17, 2.5f, 400, 500, "test");
		this.imageName="alien-medium-unit-image";
	...
	{
		this.displayTexture = MapUnitTextureContainer.findRegionAlienMediumUnit(atlasKey);
	}
	@Override
	public void schemeActivationEvent() {
		SoundEffectManager.playSmallClickSound();
		this.builder.addToQueue(new AlienMediumUnit(this.getFaction(), mapView, builder));		
	}
	@Override
